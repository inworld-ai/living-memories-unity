...
{
    /// <summary>
    /// Utility class for converting between different audio formats and handling WAV file operations.
    /// Provides methods for converting Unity AudioClips to Inworld audio chunks and vice versa,
    /// as well as WAV file reading and writing functionality.
    /// </summary>
    public class WavUtility
    {
        ...
        /// <summary>
        /// Generates an InworldAudioChunk from Unity's AudioClip data.
        /// Extracts audio samples and converts them to the format required by the Inworld framework.
        /// </summary>
        /// <param name="input">The Unity AudioClip to convert.</param>
        /// <returns>An InworldAudioChunk containing the audio data ready for processing.</returns>
        public static AudioChunk GenerateAudioChunk(AudioClip input)
        {
            float[] samples = new float[input.samples * input.channels];
            input.GetData(samples, 0);
            
            AudioChunk chunk = new AudioChunk();
            chunk.SampleRate = input.frequency;
            chunk.Data = new InworldVector<float>();
            chunk.Data.AddRange(samples.ToList());
            return chunk;
        }

        /// <summary>
        /// Generates a Unity AudioClip from an InworldAudioChunk.
        /// Converts Inworld audio data back to Unity's native audio format for playback.
        /// </summary>
        /// <param name="chunk">The InworldAudioChunk containing audio data to convert.</param>
        /// <param name="audioName">The name to assign to the created AudioClip.</param>
        /// <returns>A Unity AudioClip ready for playback, or null if the chunk is invalid.</returns>
        public static AudioClip GenerateAudioClip(AudioChunk chunk, string audioName)
        {
            if (chunk == null || !chunk.IsValid)
                return null;
            int sampleRate = chunk.SampleRate;
            InworldVector<float> data = chunk.Data;
            int dataCount = data.Size;
            List<float> dataList = data.ToList();
            AudioClip audioClip = AudioClip.Create(audioName, dataCount, 1, sampleRate, false);
            audioClip.SetData(dataList.ToArray(), 0);
            return audioClip;
        }
        ...
        /// <summary>
        /// Converts a byte array containing WAV file data into a Unity AudioClip.
        /// Supports PCM and WaveFormatExtensible uncompressed formats with 8, 16, 24, or 32-bit depth.
        /// </summary>
        /// <param name="fileBytes">The byte array containing the WAV file data to convert.</param>
        /// <param name="offsetSamples">The sample offset to apply when setting the audio data.</param>
        /// <param name="name">The name to assign to the created AudioClip.</param>
        /// <returns>A Unity AudioClip created from the WAV data.</returns>
        /// <exception cref="Exception">Thrown when an unsupported bit depth is encountered.</exception>
        ...
        /// <summary>
        /// Converts AudioClip float data to 16-bit integer format and writes it to a byte array.
        /// This conversion is required for sending audio data to the Inworld server, which expects 16-bit PCM format.
        /// </summary>
        /// <param name="input">The audio clip data as normalized float values (-1.0 to 1.0).</param>
        /// <param name="size">The number of samples to convert from the input array.</param>
        /// <param name="output">The output byte array to write the converted 16-bit audio data to.</param>
        ...
        /// <summary>
        /// Convert the short array to float array.
        /// </summary>
        /// <param name="input">short array</param>
